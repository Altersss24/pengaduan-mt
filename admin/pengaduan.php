...
$judul_halaman = 'Kelola Pengaduan';
$subjudul_halaman = 'Verifikasi dan pantau seluruh pengaduan masuk';
...
$list = mysqli_query($koneksi, "
    SELECT p.*, k.nama_kategori, u.nama AS pelapor, pet.nama AS nama_petugas
    FROM pengaduan p
    JOIN kategori k ON k.id_kategori = p.id_kategori
    JOIN user u ON u.id_user = p.id_user
    LEFT JOIN user pet ON pet.id_user = p.id_petugas
...
<div class="app-shell">
    ...
    <main class="main-content">
        ...
        <div class="page-body">
            <?php tampilkanFlash(); ?>
            <div class="card">
                <div class="card-header"><h2><i class="bi bi-card-checklist"></i> Semua Pengaduan</h2></div>
                <div class="card-body">
                    <form method="GET" style="display:flex; gap:.8rem; margin-bottom:1.3rem; flex-wrap:wrap;">
                        <input type="text" name="q" class="form-control" style="max-width:260px;" placeholder="Cari judul..." value="<?= htmlspecialchars($keyword) ?>">
                    ...
                    <table class="data-table">
                        <thead><tr><th>Judul</th><th>Pelapor</th><th>Kategori</th><th>Petugas</th><th>Tanggal</th><th>Status</th><th></th></tr></thead>
                        <tbody>
                        <?php while ($row = mysqli_fetch_assoc($list)): ?>
                            <tr>
                                ...
                                <td><a href="verifikasi.php?id=<?= $row['id_pengaduan'] ?>" class="btn btn-outline btn-sm">Kelola</a></td>
                            </tr>
                        <?php endwhile; ?>
                        </tbody>
                    </table>
